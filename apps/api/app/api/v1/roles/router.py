...
from runtime.models import Role, User, Permission, RolePermission, Tenant, Organization
from apps.api.app.modules.roles.schemas import RoleResponse, RoleCreateRequest
import uuid
...
def _ensure_tenant(db: Session, tenant_uuid: uuid.UUID):
    ...
    if not tenant:
        ...
        tenant = Tenant(id=tenant_uuid, organization_id=org_id, code="default_tenant", name="Default Tenant")
        try:
            db.add(org)
            db.add(tenant)
            db.commit()
...
def get_roles(db: Session = Depends(get_db_session), user: Dict[str, Any] = Depends(require_permissions("read"))):
    ...
    responses = []
    for r in roles:
        perms = db.query(Permission.code).join(RolePermission, Permission.id == RolePermission.permission_id).filter(RolePermission.role_id == r.id).all()
...
def create_role(request: RoleCreateRequest, db: Session = Depends(get_db_session), user: Dict[str, Any] = Depends(require_permissions("manage_users"))):
    """Create a new role."""
    raw_tenant_id = user.get("tenant_id")
    try:
        tenant_uuid = uuid.UUID(raw_tenant_id)
    ...
    for perm_code in request.permissions:
        ...
        if not permission:
            ...
            permission = Permission(id=uuid.uuid4(), code=perm_code, resource=resource, action=action)
            db.add(permission)
            db.flush()
    ...
    db.commit()
    AuditLogger.log_event("create_role", user.get("sub"), role.name, "success", tenant_id=raw_tenant_id)
