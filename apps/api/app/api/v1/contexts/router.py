...
class SemanticSearchRequest(BaseModel):
    """Schema for semantic search request against context node."""
    query: str
...
def get_context(
    ...
    try:
        ...
        logger.exception("Failed to get context %s", context_id)
...
def delete_context(
    ...
    try:
        ...
        logger.exception("Failed to delete context %s", context_id)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
def get_context_embeddings(
    ...
    try:
        ...
        logger.exception("Failed to get embeddings for context %s", context_id)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
def search_context_embeddings(
    ...
    try:
        return context_service.search_context_embeddings(
            context_id, request.query, engine
        )
    except Exception as e:
        logger.exception("Failed to search context %s", context_id)
        raise HTTPException(status_code=500, detail=str(e)) from e
