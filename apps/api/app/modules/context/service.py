...
from apps.api.app.modules.context.schemas import ContextCreateRequest, ContextDetailResponse, ContextMetadataBulkRequest
from runtime.db import SessionLocal
from runtime.models import ContextProfile, Workspace, Tenant
import uuid
import json
...
class ContextService:
    ...
    def create_context(self, request: ContextCreateRequest, engine: MemoryEngine, org_id: str = "tenant_1") -> dict:
    ...
    def get_context_by_id(self, context_id: str, engine: MemoryEngine) -> ContextDetailResponse:
        ...
        results = engine.db.graph.execute_command("sql", "SELECT * FROM ContextNode WHERE id = :id", {"id": context_id})
        if not results:
    ...
    def delete_context(self, context_id: str, engine: MemoryEngine):
        ...
        for cid in ids:
            engine.db.graph.execute_command("sql", "DELETE FROM ContextNode WHERE id = :id", {"id": cid})
    ...
    def bulk_add_metadata(self, request: ContextMetadataBulkRequest, engine: MemoryEngine) -> dict:
        """Adds a key-value pair to the metadata_json property of the specified ContextNodes."""
        import json
        
        if request.apply_to_all:
            results = engine.db.graph.execute_command("sql", "SELECT id, metadata_json FROM ContextNode")
        else:
            ids_str = ", ".join([f"'{cid}'" for cid in request.context_ids])
            if not ids_str:
                return {"status": "success", "updated_count": 0}
            results = engine.db.graph.execute_command("sql", f"SELECT id, metadata_json FROM ContextNode WHERE id IN [{ids_str}]")
    ...
    def get_all_metadata(self, engine: MemoryEngine) -> list:
