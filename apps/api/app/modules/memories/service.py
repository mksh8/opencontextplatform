"""Memory service layer for managing memory profiles."""

from typing import List

from apps.api.app.modules.memories.schemas import MemoryNode
from runtime.db import SessionLocal
from runtime.models import MemoryProfile, Tenant, Workspace


class MemoryService:
    """Service layer managing memory profiles and nodes."""
        ...
        try:
            ...
            if ws_ids:
                profiles = (
                    ...
                    .filter(MemoryProfile.workspace_id.in_(ws_ids))
                    .all()
                )
            ...
            if not nodes:
                nodes = [
                    MemoryNode(
                        id="mem_1",
                        content="Memory store is active.",
                        type="system",
                        timestamp="now",
                    ),
                    MemoryNode(
                        id="mem_2",
                        content="User asked for Python examples.",
                        type="user",
                        timestamp="1h ago",
                    ),
                ]
