...
from apps.api.app.modules.connectors.schemas import Connector, ConnectorCreateRequest, ConnectorSyncResponse, IngestionJob, IngestionJobListResponse
from sqlalchemy.orm import Session
from runtime.models import ConnectorConfig, ConnectorType, Workspace, Tenant
...
class ConnectorService:
    def get_connectors(self, org_id: str, db: Session) -> List[Connector]:
        ...
        results = []
        if ws_ids:
            results = db.query(ConnectorConfig).filter(ConnectorConfig.workspace_id.in_(ws_ids)).all()
            
        connectors = []
        for row in results:
            c_type = db.query(ConnectorType).filter(ConnectorType.id == row.connector_type_id).first()
            type_code = c_type.code if c_type else "unknown"
            
            # We parse some metadata out of configuration if needed
    ...
    def create_connector(self, org_id: str, request: ConnectorCreateRequest, db: Session) -> Connector:
        ...
        ctype = db.query(ConnectorType).filter(ConnectorType.code == request.type).first()
        if not ctype:
            ctype = ConnectorType(id=uuid.uuid4(), code=request.type, name=request.type.capitalize())
            db.add(ctype)
        ...
        if not workspace:
            workspace = Workspace(id=uuid.uuid4(), tenant_id=tenant_id, name="Default Workspace", slug="default")
            db.add(workspace)
            db.flush()
            
        conn_id = uuid.uuid4()
        # Encrypt the credentials explicitly in the configuration JSON for now
        # Ideally we'd store the raw credentials in the secrets schema
        config_dict = {
    ...
    def sync_connector(self, connector_id: str, db: Session, arcadedb: ArcadeDBRepository) -> ConnectorSyncResponse:
        ...
        c_type = db.query(ConnectorType).filter(ConnectorType.id == record.connector_type_id).first()
        ...
        # We must reassign the dictionary or mutate the dict properly for SQLAlchemy JSONB
    ...
    def get_jobs(self, db: Session) -> IngestionJobListResponse:
        """Retrieves list of active and past ingestion jobs (Intelligent Mock)."""
        ...
        for idx, conn in enumerate(connectors):
            ...
            c_type = db.query(ConnectorType).filter(ConnectorType.id == conn.connector_type_id).first()
            type_code = c_type.code if c_type else "unknown"
