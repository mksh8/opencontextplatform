...
from runtime.models import ProviderConfig, ProviderType, Workspace, Tenant
...
class ProviderService:
    def get_providers(self, org_id: str, db: Session) -> List[Provider]:
        ...
        # Find all workspaces for this org (tenant)
        # Note: org_id in the old API maps to Tenant in the new schema.
        ...
        results = []
        if ws_ids:
            results = db.query(ProviderConfig).filter(ProviderConfig.workspace_id.in_(ws_ids)).all()
            
        providers = []
        for row in results:
            prov_type = db.query(ProviderType).filter(ProviderType.id == row.provider_type_id).first()
        ...
        # Add mocks if empty so the UI looks good initially
        if len(providers) == 0:
            providers = [
                Provider(id="prov_1", name="OpenAI Default", url="api.openai.com", model="gpt-4o", status="Active", usage="1.2M tokens")
            ]
            
        return providers

    def create_provider(self, org_id: str, request: ProviderCreateRequest, db: Session) -> Provider:
        """Saves a new provider to PostgreSQL."""
        # Get or create provider type
        ptype = db.query(ProviderType).filter(ProviderType.code == request.provider_type).first()
        if not ptype:
            ptype = ProviderType(id=uuid.uuid4(), code=request.provider_type, name=request.provider_type.capitalize())
            db.add(ptype)
            
        # Find a default workspace for this org
        tenants = db.query(Tenant).filter(Tenant.organization_id == org_id).all()
        ...
        # Determine some default models to store based on provider_type
        default_models = []
