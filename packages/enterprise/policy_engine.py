...
class IPolicyEngine(ABC):
    @abstractmethod
    def evaluate(self, identity: Dict[str, Any], context_obj: Dict[str, Any], action: str) -> bool:
        pass
...
class OPAPolicyEngine(IPolicyEngine):
    ...
    def evaluate(self, identity: Dict[str, Any], context_obj: Dict[str, Any], action: str) -> bool:
        ...
        print(f"[OPA] Evaluated {action} on {context_obj.get('id')} for {identity.get('sub')}")
